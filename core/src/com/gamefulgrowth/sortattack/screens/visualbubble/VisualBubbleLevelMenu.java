...
public class VisualBubbleLevelMenu implements Screen {

	private String ALGORITHM_TYPE = "Bubble Sort Visualization";
	public static final int LEVEL_NUMBER = 3;
	private static final String HEADING_TEXT = "Bubble Sort Visualization Menu";
	private Stage stage;
	private Skin skin;
	...
	public void show() {
		...
		btnBack.addListener(new ClickListener() {
			...
			public void clicked(InputEvent event, float x, float y) {
				Sort.writeLogs(ALGORITHM_TYPE + ": Quit from Bubble Sort Level Selection Menu");
	...
	private void setEachLevelButtonAction() {
		...
		textButton.addListener(new ClickListener() {
			@Override
			public void clicked(InputEvent event, float x, float y) {
				Sort.writeLogs(ALGORITHM_TYPE + ": Bubble Sort Introduction is selected");
				Sort.assetManager.get("sounds/menuclick.wav", Sound.class)
						.play();
				Sort.screens.put(
						VisualBubbleLevelIntroduction.class.toString(),
						new VisualBubbleLevelIntroduction());
				((Game) Gdx.app.getApplicationListener())
						.setScreen(Sort.screens
								.get(VisualBubbleLevelIntroduction.class
										.toString()));
			}
		});
		...
		textButton.addListener(new ClickListener() {
			@Override
			public void clicked(InputEvent event, float x, float y) {
				Sort.writeLogs(ALGORITHM_TYPE + ": Bubble Sort Tutorial is selected");
				Sort.assetManager.get("sounds/menuclick.wav", Sound.class)
						.play();
				Sort.screens.put(VisualBubbleLevelTutorial.class.toString(),
						new VisualBubbleLevelTutorial());
				((Game) Gdx.app.getApplicationListener())
						.setScreen(Sort.screens
								.get(VisualBubbleLevelTutorial.class
										.toString()));
			}
		});
		...
		textButton.addListener(new ClickListener() {
			@Override
			public void clicked(InputEvent event, float x, float y) {
				Sort.writeLogs(ALGORITHM_TYPE + ": Bubble Sort Configuration is selected");
				Sort.assetManager.get("sounds/menuclick.wav", Sound.class)
						.play();
				Sort.screens.put(VisualBubbleConfiguration.class.toString(),
						new VisualBubbleConfiguration());
				((Game) Gdx.app.getApplicationListener())
						.setScreen(Sort.screens
								.get(VisualBubbleConfiguration.class
										.toString()));
			}
		});
